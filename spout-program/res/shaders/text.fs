...
{   
    ...
    vec4 glyphSample= vec4(1.0, 1.0, 1.0, texture(textArray, vec3(glyphTexCoords.xy, glyphTexCoords.z)).r);
    color = inputSample * glyphSample;
    if(glyphTexCoords.z > 0.1 && glyphSample.w == 0.0) {
        color = bgColor;
    }
}